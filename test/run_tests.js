...
// typically you want to wait until you establish a db connection,
// if you have a db, also grab dependencies you need to pass to test specs.

let testFiles = fs.readdirSync(__dirname + '/test_specs');
testFiles.forEach((file) => {
  require('./test_specs/' + file)(); // pass along any dependencies
});

// start mocha tests (only works if --delay flag is set)
run();
